...
adjfuturesdir = os.path.join(datadir,'data', 'futures', 'adjusted_prices_csv')
...
def wrangle_parquet(instrument, parquet_dir_adjusted=parquet_dir_adjusted_):
    """
    Process parquet file data for a given instrument.
    
    Args:
        instrument (str): The instrument identifier
        parquet_dir_adjusted (str): Directory containing parquet files
        
    Returns:
        tuple: (DataFrame with daily data, DataFrame with weekly data)
...
for column in daily_to_separate.columns[1:]:
    ...
    to_write.to_csv(os.path.join(adjfuturesdir,column+'.csv'), index=False)

plot_separate, plot_log = 0,0

if plot_separate:
    for column in daily.columns:
        test=pd.read_csv(os.path.join(adjfuturesdir,column+'.csv'))
        if plot_log:
...
file_path = '/Users/stevenmichiels/pst/assetprices.csv'
data = pd.read_csv(file_path)

start = daily[daily.index>'1999-08-01'].iloc[0,1]

plt.plot(daily[daily.index>'1999-08-01']['SPX'])
